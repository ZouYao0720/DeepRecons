...
class DataHandler:
    ...
    def __init__(self, lr_dir, hr_dir, patch_size, scale, n_validation_samples=None):
        ...
        print("n_validation_samples", n_validation_samples)
        self._make_img_list()
        self._check_dataset()
    ...
    def _crop_imgs(self, imgs, batch_size, flatness):
        ...
        crops['lr'] = np.array(crops['lr']) # batch size, patch size * patch size 
        crops['hr'] = np.array(crops['hr']) # batch size, patch size * patch size
        return crops
